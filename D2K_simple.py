...
#-*- coding: utf-8 -*-
"""
Created on Wed Mar 01 09:28:18 2017

@author: Balint Tillman
"""
...
def test_isvaliddirectedjointdegree():
...
def test_directedjointdegreemodel():
...
def is_valid_directed_joint_degree(in_degrees, out_degrees, nkk):
    ...
    References
    ----------
    [1] B. Tillman, A. Markopoulou, C. T. Butts & M. Gjoka (2017),
        Construction of Directed 2K Graphs. arXiv preprint arXiv:1703.07340.
    """
    V = {}  # number of nodes with in/out degree.
    forbidden = {}
    if len(in_degrees) != len(out_degrees):
        print("condition0 error")
        return False
    ...
    for k in nkk:
        for l in nkk[k]:
            ...
            if val > 0:
                ...
                # print(k,l)
                # print(val)
                # print(forbidden.get((k, l), 0))
                # print(V[(k, 1)])
                # print(V[(l, 0)])
                # if val + forbidden.get((k, l), 0) > V[(k, 1)] * V[(l, 0)]:
                #     print("condition3 error",k,l)
                #     return False
                try:
                    if val + forbidden.get((k, l), 0) > V[(k, 1)] * V[(l, 0)]:
                        # nkk[k][l]=V[(k, 1)] * V[(l, 0)]
    ...
    for s in S:
        if not float(S[s]) / s[0] == V[s]:  # condition 2
            print("condition2 error:node",s,float(S[s]) / s[0],V[s])
            return False
...
def directed_joint_degree_model(in_degrees, out_degrees, nkk):
    ...
    >>> out_degrees = [1, 1, 1, 1]
    >>> nkk = {1:{1:2,2:2}}
    >>> G=nx.directed_joint_degree_model(in_degrees, out_degrees, nkk)
    >>>
    """
...
#        raise nx.NetworkXError(msg)
    ...
    for idx, o in enumerate(out_degrees):
        ...
        '''
        #             赋值语句代码等价于
        #             non_chords[(o, in_degrees[idx])]=0
        #             non_chords[(o, in_degrees[idx])]=non_chords[(o, in_degrees[idx])]+1
        #         
        '''
        idx = int(idx)
        if o > 0:
    ...
    for k in nkk:
        for l in nkk[k]:
            ...
            if (n_edges_add > 0):
                ...
                k_len = nk_out[k]
                l_len = nk_in[l]
                # n_edges_add需要添加的边的个数
                #在(0,k_len * l_len)之间，取n_edges_add+ non_chords.get((k, l), 0)长度的随机数
                #non_chords.get((k, l), 0)：如果(k,l)有值则返回值，否则返回0
                try:
                    chords_sample = random.sample(range(k_len * l_len),n_edges_add+ non_chords.get((k, l), 0))
                except:
                    print(k,l,n_edges_add,"and chords_sample:",chords_sample)
                    print(k_len,l_len,n_edges_add,non_chords.get((k, l), 0))


                num = 0
                while len(chords) < n_edges_add:
                    i = h_degree_nodelist_out[k][chords_sample[num] % nk_out[k]]
                    j = h_degree_nodelist_in[l][chords_sample[num] // nk_out[k]]
                    # print(len(h_degree_nodelist_out[k]))
                    # print(chords_sample[num] % nk_out[k])
                    ...
                    if i != j:
                        chords.add((i, j))
                    # else:
                    #     i=h_degree_nodelist_out[k][chords_sample[num+1] % nk_out[k]]
                    #     chords.add((i, j))
